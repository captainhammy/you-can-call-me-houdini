...
def test_OnUninstall(mocker: MockerFixture, execute_houdini_script: Callable[[str, dict], None]) -> None:
    """Test the OnUninstall.py script."""
    mock_emit = mocker.patch.object(CallbackManager, "emit")

    execute_houdini_script("OnUninstall", {"node": hou.node("/obj")})

    mock_emit.assert_called_with(HoudiniNodeEvent.OnUninstall, {"node": hou.node("/obj")})
