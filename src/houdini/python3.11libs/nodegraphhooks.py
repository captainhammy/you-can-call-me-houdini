...
def _execute_keyboard_callbacks(uievent: KeyboardEvent, pending_actions: list) -> tuple[None, bool]:
    scriptargs = {
...
def _handle_paste_event(uievent: KeyboardEvent) -> None:
    ...
    with hou.undos.group("Paste from clipboard"):
        ...
        if eventtype != "parentkeyhit":
            mousepos = editor.screenBounds().center() if eventtype == "menukeyhit" else uievent.mousepos
...
def _is_paste_event(uievent: KeyboardEvent) -> bool:
    ...
    return nodegraphdisplay.setKeyPrompt(uievent.editor, uievent.key, "h.paste", uievent.eventtype)
